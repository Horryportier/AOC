(*
# TODO:
- [ ] get numbers from every line (number is made from first and last of every line)
- [ ] add them
*)
let input = 
"8eight1
...
(* let print_list f l = List.iter (fun x ->  Printf.printf f x) l *)
...
let part1 i = 
    ...
    |> map ( fun x -> 
        (nth x 0) :: (nth x (length x - 1))   :: []
    ) |> map (
        fun d -> String.concat "" ( List.map (String.make 1) d ) |> int_of_string
    ) |> fold_left (+) 0
        

let ()  = print_string "PART1:\n\t"; print_int (part1 input)
